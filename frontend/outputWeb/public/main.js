// main.js – Modified for integration with outputWeb
// Changes made:
// 1. Wrap all code in function main() and assign it to window.main.
// 2. Use a fixed URL for the .splat file instead of URLSearchParams.
// 3. Assume a <canvas id="canvas"></canvas> exists.
// 4. (Retain most of the original functionality.)

function main() {
    // Fixed URL: always load "truck.splat" from the base URL.
    const splatFile = "truck.splat";
    const baseURL = "https://huggingface.co/cakewalk/splat-data/resolve/main/";
    const url = new URL(splatFile, baseURL);
    console.log("Loading .splat file from:", url.toString());
  
    // Cameras data – unchanged
    let cameras = [
      {
        id: 0,
        img_name: "00001",
    ...
    // multiply4, invert4, rotate4, and translate4 functions remain unchanged.
    ...
    async function mainFunction() {
        ...
        // Use the fixed URL 'url' defined at the top.
            ...
            for (let gamepad of gamepads) {
                ...
                if (gamepad.buttons[12].pressed || gamepad.buttons[13].pressed) {
                    inv = translate4(
                        inv,
                        0,
                        -moveSpeed * (gamepad.buttons[12].pressed - gamepad.buttons[13].pressed),
                        0,
                    );
                    carousel = false;
                }
                ...
                if (gamepad.buttons[14].pressed || gamepad.buttons[15].pressed) {
                    inv = translate4(
                        inv,
                        -moveSpeed * (gamepad.buttons[14].pressed - gamepad.buttons[15].pressed),
                        0,
                        0,
                    );
                    carousel = false;
    ...
    mainFunction().catch((err) => {
        const spinner = document.getElementById("spinner");
        if (spinner) {
        spinner.style.display = "none"; // or "" depending on the context
        }
        document.getElementById("message").innerText = err.toString();
    });
    }
    
    // Expose the main function so it can be called from our React component.
    window.main = main;
